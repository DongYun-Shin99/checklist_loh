# 업무 체크리스트

윈도우용 업무 체크리스트 관리 프로그램. 프리셋(템플릿)으로 반복 작업을 정의하고,
패치 단위로 국가(한국/대만/일본)별 체크리스트를 만들어 진행 상황을 관리한다.

## 구조 (3단계)

```
패치 (패치일 + 국가 + 전체 진행률 %)
 └─ 체크리스트 (프리셋에서 생성 또는 빈 체크리스트로 직접 작성)
     └─ 항목 (체크 / 폴더·파일 경로 / 메모)
         └─ 하위 항목 (1단계, 전부 완료되면 부모 자동 완료)
```

## 주요 기능

- **설정**: 빌드(국가)별 기본 폴더(SVN 트렁크 루트) 등록. 예: `C:\00.SVN\00.KR_LOH_Trunk`
- **프리셋**: 체크리스트 템플릿. 항목 경로는 기본적으로 **상대 경로 + 파일 명 한 벌**만 입력하면
  체크리스트 생성 시 `기본 폴더(국가) + 상대 경로`로 자동 조합된다.
  국가별로 구조가 다른 예외 항목은 "국가별 따로 입력" 체크로 3개국 경로를 개별 입력.
  하위 항목(1단계) 지원, ↑↓로 순서 변경, JSON 내보내기/불러오기로 공유 가능.
- **패치**: 최상위 단위. 패치일(기한) + 국가. 전체 진행률 %와 D-day 색상 표시(임박 시 주황, 초과 시 빨강).
- **체크리스트**: 패치 안에 프리셋으로 추가하면 경로가 패치 국가 것으로 확정.
  루틴 외 작업은 빈 체크리스트를 만들어 항목을 직접 추가/수정/삭제.
  - 항목 상태: 진행중 / 완료. 하위 항목이 전부 완료되면 부모 자동 완료, 부모 체크 시 하위 일괄 토글.
  - 항목별 메모, 경로를 탐색기에서 바로 열기, 없는 경로는 빨간 경고 표시
- **리소스 경로**: 아이콘 등 자주 쓰는 리소스 폴더/파일 바로가기 모음. 상대 경로 한 줄로 등록하면
  '기준 빌드' 선택에 따라 해당 국가 트렁크로 연결됨. 더블클릭으로 탐색기에서 열기, 없는 경로는 빨간 경고.
  프리셋 항목/체크리스트 항목에서 [리소스] 버튼으로 등록된 경로를 바로 불러올 수 있음.
- **보관함**: 패치 안 모든 체크리스트가 완료되면 패치째로 보관. 펼치면 국가별 체크리스트가 보이고
  읽기 전용 열람, 복원, 삭제 가능.
- **자동 저장**: 모든 변경은 즉시 `data/app_data.json`에 저장.

## 실행 (개발)

```
pip install -r requirements.txt
python main.py
```

## 윈도우용 exe 빌드

```
pip install pyinstaller
pyinstaller --onefile --windowed --name checklist main.py
```

`dist/checklist.exe` 생성. 데이터는 exe 옆의 `data/` 폴더에 저장된다.

## 테스트

```
QT_QPA_PLATFORM=offscreen python tests/smoke_test.py [스크린샷_폴더]
```

## 구조

```
main.py                      진입점
app/
  models.py                  프리셋/패치/체크리스트 데이터 모델
  storage.py                 JSON 저장소 (자동 저장)
  utils.py                   D-day 계산, 탐색기 열기
  widgets.py                 공용 위젯 (국가 뱃지)
  style.py                   QSS 스타일시트
  dialogs.py                 새 패치 / 체크리스트 추가 / 보관함 상세 다이얼로그
  main_window.py             사이드바 + 3단계 페이지 전환
  pages/
    patches.py               진행 중 패치 목록 (메인)
    patch_detail.py          패치 상세 (체크리스트 목록)
    checklist_detail.py      체크리스트 상세 (체크/경로/메모)
    presets.py               프리셋 목록 + 편집기 + 내보내기/불러오기
    resources.py             리소스 경로 바로가기
    archive.py               보관함 (완료된 패치)
    settings.py              설정 (빌드별 기본 폴더)
```
